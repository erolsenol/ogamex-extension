...
(function () {
  ...
  class Hand {
    constructor(cards, name, game, canDisqualify) {
      ...
      for (var i = 0; i < this.cardPool.length; i++) {
        ...
        if (card.rank === -1) {
          ...
          (obj1 = this.values)[(key1 = card.rank)] ||
            (obj1[key1] = []);
    ...
    qualifiesHigh() {
      ...
      return (
        this.compare(
          Hand.solve(this.game.lowestQualified, this.game),
        ) <= 0
      );
    }
  ...
  class NaturalRoyalFlush extends RoyalFlush {
    ...
    solve() {
      ...
      if (result && this.cards) {
        for (
          i = 0;
          i < this.game.sfQualify && i < this.cards.length;
          i++
        ) {
    ...
    solve() {
      ...
      if (this.cards.length === 4) {
        for (i = 0; i < this.values.length; i++) {
          ...
          if (
            cards &&
            this.cards[0].wildValue === cards[0].wildValue
          ) {
            continue;
          }
      ...
      for (; i > 0; i--) {
        ...
        for (var j = 0; j < cardsToCheck.length; j++) {
          ...
          if (diff === null) {
            ...
            checkHandLength <
            gapCount + diff + cardsList.length
          ) {
            break;
          } else if (diff > 0) {
            cardsList.push(card);
        ...
        if (
          this.game.sfQualify - gapCards.length <=
          wildCards.length
        ) {
          break;
        }
  ...
  class PaiGowPokerHelper {
    ...
    qualifiesValid() {
      ...
      return !(
        compareHands.length === 1 && compareHands[0] === this.loHand
      );
    }
    ...
    static solve(fullHand) {
      var result = new PaiGowPokerHelper(
        (fullHand = fullHand || ['']),
      );
